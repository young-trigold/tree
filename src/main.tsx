...
import './main.less';
import { type FunctionComponent, useState, useRef, CSSProperties, useEffect } from 'react';
...
const Tree: FunctionComponent<TreeConfig> = (props) => {
    ...
    useEffect(() => {
         ...
         * @returns {Point} 终点
         */
        const drawBranch = (startPoint: Point, angle: number, length: number, width: number, color: string) => {
            context.beginPath();
        ...
        const drawTreeWithMainBranch = (
            ...
            // 主干
            const endPoint = drawBranch(rootPosition, Math.PI / 2, mainBranchHeight, mainBranchThickness, branchColor);
            ...
            const drawTree = (curPoint: Point, curAngle: number, curLength: number, curThickness: number) => {
                // 树枝生长角度变为下方
                const back = curAngle < 0 || (curAngle > Math.PI);
                // 树枝生长位置超出画布
                const beyond = Math.abs(curPoint.x) > canvas.width / 2 || (Math.abs(curPoint.y) > canvas.height);
                // 树枝太短
                ...
                if (back || beyond || short) {
                    ...
                    if (short) {
                        // 画花
                        if (hasFlower && Math.round(Math.random())) drawFlower(curPoint, 20 * gaussianRandom(), flowerColor);
                    }
                    return;
                }
...
const App = () => {
    ...
    const [backgroundColor, setBackgroundColor] = useState(treeConfig.backgroundColor);

    const onBackgroundColorChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
    ...
    const onBranchColorChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
    ...
    const onHasFlowerChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setHasFlower(event.target.checked);
    };

    const [flowerColor, setFlowerColor] = useState(treeConfig.flowerColor);

    const onFlowerColorChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setFlowerColor(event.target.value);
    };

    const [rootPosition, setRootPosition] = useState(treeConfig.rootPosition);

    const onRootPositionXChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        const x = Number(event.target.value);
        setRootPosition((preRootPosition) => ({ ...preRootPosition, x }));
    };

    const onRootPositionYChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        const y = Number(event.target.value);
        setRootPosition((preRootPosition) => ({ ...preRootPosition, y }));
    };

    const [mainBranchHeight, setMainBranchHeight] = useState(treeConfig.mainBranchHeight);

    const onMainBranchHeightChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setMainBranchHeight(Number(event.target.value));
    };

    const [mainBranchThickness, setMainBranchThickness] = useState(treeConfig.mainBranchThickness);

    const onMainBranchThicknessChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setMainBranchThickness(Number(event.target.value));
    };

    const [angleDelta, setAngleDelta] = useState(treeConfig.angleDelta);

    const onAngleDeltaChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setAngleDelta(Number(event.target.value));
    };

    const [branchLengthDecreaseRatio, setBranchLengthDecreaseRatio] = useState(treeConfig.branchLengthDecreaseRatio);

    const onBranchLengthDecreaseRatioChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setBranchLengthDecreaseRatio(Number(event.target.value));
    };

    const [branchThicknessDecreaseRatio, setBranchThicknessDecreaseRatio] = useState(treeConfig.branchThicknessDecreaseRatio);

    const onBranchThicknessDecreaseRatioChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setBranchThicknessDecreaseRatio(Number(event.target.value));
    };

    const [random, setRandom] = useState(treeConfig.random);

    const onRandomChange: React.ChangeEventHandler<HTMLInputElement> = (event) => {
            ...
            <div className='formContainer'>
                <form className="form" style={formStyle}>
                    <div className="field">
                        ...
                        <input value={backgroundColor} onChange={onBackgroundColorChange} type="color" id="backgroundColor" />
                    </div>
                    <div className="field">
                        <label htmlFor="branchColor">树枝的颜色</label>
                        <input value={branchColor} onChange={onBranchColorChange} type="color" id="branchColor" />
                    </div>
                    <div className='field'>
                        <input checked={(hasFlower)} onChange={onHasFlowerChange} type="checkbox" id="hasFlower" />
                        <label htmlFor="hasFlower">需要画花</label>
                    </div>
                    {hasFlower ? <div className="field">
                        <label htmlFor="flowerColor">花的颜色</label>
                        <input value={flowerColor} onChange={onFlowerColorChange} type="color" id="flowerColor" />
                    </div> : null}
                    <div className="field">
                        <label htmlFor="rootPosition">树根的位置（x, y），原点在屏幕正下</label>
                        <input min={-innerWidth / 2 * devicePixelRatio} max={innerWidth / 2 * devicePixelRatio} value={rootPosition.x} onChange={onRootPositionXChange} type="number" />
                        <input min={0} max={innerHeight * devicePixelRatio} value={rootPosition.y} onChange={onRootPositionYChange} type="number" />
                    </div>
                    <div className="field">
                        <label htmlFor="mainBranchHeight">主干的高度</label>
                        <input min={0} max={innerHeight * devicePixelRatio} value={mainBranchHeight} onChange={onMainBranchHeightChange} type="number" id="mainBrachHeight" />
                    </div>
                    <div className="field">
                        <label htmlFor="mainBranchThickness">主干的粗细</label>
                        <input min={0} max={innerHeight * devicePixelRatio} value={mainBranchThickness} onChange={onMainBranchThicknessChange} type="number" id="mainBranchThickness" />
                    </div>
                    <div className="field">
                        <label htmlFor="angleDelta">每次画树枝角度变化值（0 ~ PI / 2）</label>
                        <input value={angleDelta} onChange={onAngleDeltaChange} type="number" id="angleDelta" />
                    </div>
                    <div className="field">
                        <label htmlFor="branchLengthDecreaseRatio">每次画树枝长度衰减比率（0~1）</label>
                        <input style={{width: '100px'}} min={0} max={1} value={branchLengthDecreaseRatio} onChange={onBranchLengthDecreaseRatioChange} type="number" id="branchLengthDecreaseRatio" />
                    </div>
                    <div className="field">
                        <label htmlFor="branchThicknessDecreaseRatio">每次画树枝粗细度衰减比率（0～1）</label>
                        <input style={{width: '100px'}} min={0} max={1} value={branchThicknessDecreaseRatio} onChange={onBranchThicknessDecreaseRatioChange} type="number" id="branchThicknessDecreaseRatio" />
                    </div>
                    <div className="field">
                        <input checked={random} onChange={onRandomChange} type="checkbox" id="random" />
                        <label htmlFor="random">添加随机性</label>
                    </div>
                    <div className="field">
                        <button onClick={onSubmit} className="submit-button" type="submit">提交</button>
                        <button style={{ marginLeft: '24px' }} onClick={onReset} className="reset-button" type="button">重置</button>
                    </div>
                </form>

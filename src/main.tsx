    ...
    useEffect(() => {
        ...
        const drawTreeWithMainBranch = (
            ...
            const drawTree = (
                ...
                // 树枝生长位置超出画布
                const beyond =
                    Math.abs(curPoint.x) > canvas.width / 2 || Math.abs(curPoint.y) > canvas.height;
                // 树枝太短
...
const App = () => {
    ...
    const onBranchLengthDecreaseRatioChange: React.ChangeEventHandler<HTMLInputElement> = (
        event,
    ) => {
        setBranchLengthDecreaseRatio(Number(event.target.value));
    };

    const [branchThicknessDecreaseRatio, setBranchThicknessDecreaseRatio] = useState(
        treeConfig.branchThicknessDecreaseRatio,
    );

    const onBranchThicknessDecreaseRatioChange: React.ChangeEventHandler<HTMLInputElement> = (
        event,
    ) => {
